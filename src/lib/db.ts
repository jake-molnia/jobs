...
import { recordSchema, type Entry, type ListQuery, type RecordInput, type RecordPage } from "./records";

const storedRowSchema = z.object({
  id: z.string(), payload: z.string(), createdAt: z.string(), updatedAt: z.string(),
});
const countSchema = z.object({ count: z.number() });
const countsSchema = z.object({
  all: z.number(), saved: z.number(), applied: z.number(), interview: z.number(), closed: z.number(),
});

export class ConflictError extends Error {
  constructor() { super("A record with this URL already exists."); this.name = "ConflictError"; }
}

function decode(row: unknown): Entry {
  const stored = storedRowSchema.parse(row);
  const payload: unknown = JSON.parse(stored.payload);
  return recordSchema.parse({ ...z.record(z.string(), z.unknown()).parse(payload),
    id: stored.id, createdAt: stored.createdAt, updatedAt: stored.updatedAt });
}
...
export function createStore(path: string) {
  ...
  try {
    ...
    db.function("casefold", (value) => typeof value === "string" ? value.toLowerCase() : "");
    db.exec("BEGIN IMMEDIATE");
    const version = z.object({ user_version: z.number() }).parse(db.prepare("PRAGMA user_version").get()).user_version;
    if (version > 1) throw new Error("Database schema is newer than this application supports.");
    if (version === 0) {
      db.exec(`
  ...
  return {
    ...
    list(query: ListQuery): RecordPage {
      ...
      const sort = { updated: "updatedAt DESC, id ASC", deadline: "deadline IS NULL, deadline ASC, id ASC",
        organization: "organization COLLATE NOCASE ASC, id ASC" }[query.sort];
      db.exec("BEGIN");
      try {
        const records = db.prepare(`SELECT * FROM records ${where} ORDER BY ${sort} LIMIT ? OFFSET ?`)
          .all(...values, query.limit, query.offset).map(decode);
        const total = countSchema.parse(db.prepare(`SELECT count(*) AS count FROM records ${where}`).get(...values)).count;
        const counts = countsSchema.parse(db.prepare(`SELECT count(*) AS 'all',
          coalesce(sum(status = 'saved'), 0) AS saved, coalesce(sum(status = 'applied'), 0) AS applied,
          coalesce(sum(status = 'interview'), 0) AS interview, coalesce(sum(status = 'closed'), 0) AS closed FROM records`).get());
        db.exec("COMMIT");
    ...
    upsert(input: RecordInput): Entry {
      ...
      const row = db.prepare(`INSERT INTO records (id, url, title, organization, status, kind, deadline, createdAt, updatedAt, payload)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        .get(randomUUID(), input.url, input.title, input.organization, input.status, input.kind,
          input.deadline, now, now, JSON.stringify(input));
      const entry = decode(row);
      logger.info({ event: "record.upsert", recordId: entry.id }, "Record saved");
      return entry;
    },
    update(id: string, patch: Partial<RecordInput>): Entry | null {
      ...
      try {
        ...
        void recordId; void createdAt; void updatedAt;
        const next = { ...input, ...patch };
        const row = db.prepare(`UPDATE records SET url = ?, title = ?, organization = ?, status = ?,
          kind = ?, deadline = ?, updatedAt = ?, payload = ? WHERE id = ? RETURNING *`)
          .get(next.url, next.title, next.organization, next.status, next.kind, next.deadline,
            new Date().toISOString(), JSON.stringify(next), id);
        const entry = decode(row);
        db.exec("COMMIT");
        ...
        db.exec("ROLLBACK");
        if (error instanceof Error && error.message.includes("UNIQUE constraint failed: records.url")) throw new ConflictError();
        throw error;
      }
    },
    health() { db.prepare("SELECT count(*) FROM records").get(); return true; },
    close() { db.close(); },
  };
}
